...
const TechnicianPortal = ({ onClose }) => {
  ...
  // 🔑 Fresh User Extraction from LocalStorage
  const getUserData = () => {
    try {
  ...
  // 🎯 SMART TECHNICIAN DUTY MATCHING ENGINE
  const fetchTechDuties = useCallback(async () => {
    ...
    try {
      ...
      const myId = String(user._id || user.id || '').trim().toLowerCase();
      const myEmail = String(user.email || '').trim().toLowerCase();
      const myName = String(user.name || '').trim().toLowerCase();

      const myAllJobs = rawData.filter(booking => {
        // Agar assigned nahi hai toh skip
        ...
        if (typeof booking.assignedTechnician === 'object' && booking.assignedTechnician !== null) {
          assignedId = String(booking.assignedTechnician._id || booking.assignedTechnician.id || '').trim().toLowerCase();
          assignedEmail = String(booking.assignedTechnician.email || '').trim().toLowerCase();
          assignedName = String(booking.assignedTechnician.name || '').trim().toLowerCase();
        } else if (typeof booking.assignedTechnician === 'string') {
          assignedId = booking.assignedTechnician.trim().toLowerCase();
          assignedName = booking.assignedTechnician.trim().toLowerCase();
        }

        const fallbackName = String(booking.technician || '').trim().toLowerCase();

        // 🔍 Multi-Level Matching: ID, Email, Direct Name ya Substring
        const idMatch = myId && (assignedId === myId);
        const emailMatch = myEmail && (assignedEmail === myEmail);
        const nameMatch = myName && (
          assignedName === myName ||
          fallbackName === myName ||
          assignedName.includes(myName) ||
          myName.includes(assignedName) ||
          fallbackName.includes(myName) ||
          myName.includes(fallbackName)
        );
      ...
      setActionMessage("⚠️ Database sync issue. Check if backend is running.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    fetchTechDuties();
  }, [fetchTechDuties, refreshTrigger]);
  ...
  const handleAddPart = () => {
    setParts(prev => [...prev, { name: '', qty: 1, price: '' }]);
  };
            ...
            {activeJobs.map((job) => (
              <div key={job._id} style={{ padding: '18px', border: '2px solid #38bdf8', borderRadius: '12px', backgroundColor: '#f0f9ff', borderLeft: job.status === 'Accepted' ? '8px solid #2563eb' : '8px solid #f97316' }}>
                ...
                <div style={{ fontSize: '14px', color: '#334155', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  ...
                  <div>📅 <b>Scheduled:</b> {job.bookingDate || 'Today'} ({job.bookingTime || 'Morning'})</div>
                </div>
